...
export function PricingCard({ isAnnual, type }: PricingCardProps) {
	...
	return <Wrapper as="div" className={`
		...
		<ul className="list-disc list-inside text-left">
			{
				plans[type].features.map((feature, index) => <Text as="li" size="body" key={index}>{feature}</Text>)
			}
		</ul>
		<If condition={type !== "enterprise"}>
			<Wrapper as="div" className="flex flex-row justify-between items-center gap-5 p-3">
				<Text as="p" size="body-large">
					<b className="text-primary">{isAnnual ? plans[type].annual : plans[type].monthly}EGP</b>
		...
		<Button variant="primary" href="/not-implemented">
			{
				type === "personal" || "professional" ? "Go " + plans[type].name : "Contact Sales"
			}
		</Button>
	</Wrapper>
}